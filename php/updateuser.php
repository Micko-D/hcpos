...
if (isMobileDevice()) {
    ...
    $currentTime = date("H:i:s");
        ...
        if (empty($password) || $password === ""){
            $sql = "UPDATE users 
            SET fullname ='$name', email='$email', contactnum='$contact', `address`='$address' 
            WHERE userid = '$empID'";

            $auditSql = "INSERT INTO `audit` (`role`, userid, `date`, `time`, fromdev, `action`, prodname, info)
            VALUES ('$role', '$userid', '$currentDate', '$currentTime', '$device', 'Update Profile ', '', 'User')";
            ...
            $sql = "UPDATE users 
            SET accpassword='$password', fullname ='$name', email='$email', contactnum='$contact', `address`='$address' 
            WHERE userid = '$empID'";
