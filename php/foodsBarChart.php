...
$query = "
SELECT prodid, producttype, SUM(total_quantity) as total_quantity
FROM (
    SELECT p.prodid, p.producttype, SUM(o.quantity) as total_quantity
    FROM orders o
    JOIN products p ON o.productid = p.prodid
    WHERE p.producttype = 'Food' AND WEEK(o.orderdate) = WEEK(NOW())
    GROUP BY p.prodid, p.producttype
    ...
    WHERE p.producttype = 'Food' AND WEEK(oo.orderdate) = WEEK(NOW())
    GROUP BY p.prodid, p.producttype
) AS subquery
GROUP BY prodid, producttype;
";
...
while ($row = $result->fetch_assoc()) {
    ...
    if ($productNameResult) {
        ...
        // Handle query error
        die("Product name query failed: " . $conn->error);
    }
}
